...
def test_create_dist_version_exists(browser):
    ...
    splunk_test.open_pyden_search()
    results = splunk_test.run_search("| createdist version=3.7.1")
    assert "Successfully compiled Python 3.7.1" in results
    results = splunk_test.run_search("| createdist version=3.7.1")
...
def test_delete_dist_success(browser):
    ...
    splunk_test.open_pyden_search()
    results = splunk_test.run_search("| createdist version=3.7.2")
    assert "Successfully compiled Python 3.7.2" in results
    results = splunk_test.run_search("| pydelete 3.7.2")
    assert "Successfully deleted dist 3.7.2" in results


def test_delete_dist_version_not_exists(browser):
    splunk_test = splunk.SplunkTest(browser, "delete_dist_version_not_exists")
    splunk_test.open_pyden_search()
    results = splunk_test.run_search("| pydelete 3.7.3")
    assert "Version or environment 3.7.3 not found in configuration" in results
